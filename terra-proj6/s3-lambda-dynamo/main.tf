...
resource "aws_lambda_funtion" "s3_to_dynamo" {
	funtion_name = "S3toDynamoLambda"
...
resource "aws_s3_bucket_notification" "s3_notify_lambda" {
	bucket = aws_s3_bucket.my-terra-bucket.arn
	
	lambda_funtion {
	   lambda_function_arn = aws_lambda_funtion.s3_to_dynamo.arn
	   events = ["s3:ObjectCreated:*"]      		#Trigger on any object
	   filter_suffix  =  ".json"    			#only for json file extensions
	}
